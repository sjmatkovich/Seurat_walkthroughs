...
If the compute-heavy steps in this notebook have already been run and saved, this permits notebook rendering of visualizations. Otherwise, set `eval: false` in the code chunk below and set `eval: true` throught this notebook as required.
...
load(file.path(work.dir,"tut2_all-integrations-complete_both_objs.RData"))
```
...
obj_std <- FindVariableFeatures(obj)
obj_std <- ScaleData(obj)
obj_std <- RunPCA(obj)
...
## Visualize layer integration steps
